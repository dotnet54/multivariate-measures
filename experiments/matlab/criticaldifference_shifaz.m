function [cd,f] = criticaldifference_shifaz(name,s,labels,alpha)
%
% THIS file has been modified by shifaz
...
h = text(0.5*cd/(k-1), 130, 'CD', 'FontSize', 28, 'HorizontalAlignment', 'center');

for i=1:k

   text((i-1)/(k-1), 110, num2str(k-i+1), 'FontSize', 14, 'HorizontalAlignment', 'center');
...
for i=1:ceil(k/2)
   ...
   h = text(1.2, 100 - 5*(n+1)- 10*i + 5, num2str(r(i)), 'FontSize', 20, 'HorizontalAlignment', 'right');

   text(1.25, 100 - 5*(n+1) - 10*i + 4, labels{idx(i)}, 'FontSize', 18, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
...
for i=ceil(k/2)+1:k
   ...
   text(-0.2, 100 - 5*(n+1) -10*(k-i+1)+5, num2str(r(i)), 'FontSize', 20, 'HorizontalAlignment', 'left');
   text(-0.25, 100 - 5*(n+1) -10*(k-i+1)+4, labels{idx(i)}, 'FontSize', 18, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
end

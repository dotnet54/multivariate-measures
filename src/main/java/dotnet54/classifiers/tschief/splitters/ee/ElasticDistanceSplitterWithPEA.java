...
import dotnet54.measures.univariate_pea.UnivariateMeasureWithPEA;
...
public class ElasticDistanceSplitterWithPEA extends ElasticDistanceSplitter {

	protected Random rand;
	protected UnivariateMeasureWithPEA measure;
	...
	public ElasticDistanceSplitterWithPEA(TSChiefNode node){
		super(node);
...
//			measure = node.tree.treeDistanceMeasure; // support disabled with PrunedEA - unsafe cast
		}else {
			int r = rand.nextInt(options.eeEnabledDistanceMeasures.size());
			measure = UnivariateMeasureWithPEA.createUnivariateMeasure(options.eeEnabledDistanceMeasures.get(r));
		}
...
//		this.node.sdtvWeightedGini.increment(weightedGini);
	...
	private synchronized int findNearestExemplar(
		...
		for (int key : exemplars.keys()) {
			...
			long startTime = System.nanoTime();
			//NOTE using only dimension 0 - multivariate measures are implemented in another package
			dist = distance_measure.distance(query[0], exemplar[0]);
//			dist = distance_measure.distance(query[0], exemplar[0], bsf);
...
//				}
	...
	public String toString() {
		if (measure == null) {
			return "EEPEA[untrained]";
		}else {
			return "EEPEA[" + measure.toString() + "]"; //,e={" + "..." + "}
		}		
	}

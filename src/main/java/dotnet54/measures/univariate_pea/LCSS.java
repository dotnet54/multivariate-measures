...
import dotnet54.measures.univariate.DistanceTools;
import dotnet54.tscore.data.Dataset;

import java.util.Random;

import static dotnet54.measures.univariate_pea.utils.max;
...
public class LCSS extends UnivariateMeasureWithPEA {

	protected int windowSize = -1;
	protected double epsilon;

	public LCSS(Measure measureName) {
		super(measureName);
	}

	/// Check if two numbers are within epsilon (1 = similar, 0 = not similar)
	public boolean sim(double a, double b, double epsilon) {
		return abs(a - b) < epsilon;
	}

	public double distance_wrapper(double[] lines, double[] cols, double cutoff){
		return distance(lines, cols, epsilon, windowSize, cutoff);
	}
	...
	public void setRandomParams(Dataset trainData, Random rand) {
		windowSize = rand.nextInt(Math.max(((trainData.length() +1) / 4), 4));
		double stdTrain = DistanceTools.stdv_p(trainData);
		double stdFloor = stdTrain * 0.2;
		epsilon = rand.nextDouble()*(stdTrain-stdFloor)+stdFloor;
	}
}

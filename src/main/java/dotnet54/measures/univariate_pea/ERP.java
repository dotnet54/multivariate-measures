...
public class ERP extends UnivariateMeasureWithPEA {

    protected int windowSize = -1;
    protected double g;

    public ERP(Measure measureName) {
        super(measureName);
    }

    @Override
    public double distance_wrapper(double[] series1, double[] series2, double cutoff) {
        return distance(series1, series2, g, windowSize, cutoff);
    }
    ...
    public void setRandomParams(Dataset trainData, Random rand) {
        windowSize =  rand.nextInt(trainData.length() / 4); // w ~ U[0, L/4]
        double stdv = DistanceTools.stdv_p(trainData);
        g = rand.nextDouble()*.8*stdv+0.2*stdv; //[0.2*stdv,stdv]
    }
}

...
public class TWE extends UnivariateMeasureWithPEA {
	...
	public TWE(Measure measureName) {
		super(measureName);
	}

	@Override
	public double distance_wrapper(double[] series1, double[] series2, double cutoff) {
		return distance(series1, series2, nu, lambda, cutoff);
	}
	...
	public void setRandomParams(Dataset trainData, Random rand) {
		nu = twe_nuParams[rand.nextInt(twe_nuParams.length)];
		lambda = twe_lamdaParams[rand.nextInt(twe_lamdaParams.length)];
	}
}
